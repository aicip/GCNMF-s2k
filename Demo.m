%%Alireza Rahimpour 2016
%This is a simple demo and doesn't include all the details and experiments.
...
% ***different modes: 
% 
% ****Decomposition mode: parameter: training=1  
...
% 
% 
% ****Prediction mode: parameter: testing=1
...
% 
% *****using NNLS and SUM to k constraint:
% ***** fast=1 
% *****NNLS_code=1 
...
en=k;  % these two numbers should be the same for A and X. 
 constraint1=1;% for sum to k constraint
...
A=A_concat;
%[Xsub,idx]=find_independent_columns(A);
%%
showflag = 1;
mixed=X;
%[MN,Band] = size(mixed);     % column:bands, row:samples
 [M,N]=size(X);
...
%     p=19;
%     ni=k;
%    [sest, history] = group_lasso_feat_split(A, X, lambda, ni, rho, alpha);
%     %[sest,history] = group_lasso(A, X, lambda, P, rho, alpha);
...
% %
% % The input p(k) is a K-element vector giving the block sizes n_i, so that x_i
% % is in R^{n_i}.
% %
...
if spc==1
    ...
    display('Algorithm: SPARSE CODING (not using SPAMS)')
...
%%
% Average of the signal constraint + sum to k :

    if constraint2==1
            display('Imposing average constraint and sum to k ');
...
%%Plot some examples
figure,
subplot(5,2,1), plot(x1(:,st:en),'linewidth',2),title(' North Bedroom '),ylabel('Current(AMP)');xlabel('Time(MIN)');
hold on; plot(x_est1(:,1:DAY),'r','linewidth',2); legend('True','Estimate');

subplot(5,2,2), plot(x2(:,st:en),'linewidth',2),title('Master and South bedroom '),ylabel('Current(AMP)');xlabel('Time(MIN)');
hold on; plot(x_est2(:,1:DAY),'r','linewidth',2); legend('True','Estimate');

subplot(5,2,3), plot(x3(:,st:en),'linewidth',2),title('Basement Plugs and Lights'),ylabel('Current(AMP)');xlabel('Time(MIN)');
hold on; plot(x_est3(:,1:DAY),'r','linewidth',2); legend('True','Estimate');

subplot(5,2,4), plot(x4(:,st:en),'linewidth',2),title('Clothes Dryer'),ylabel('Current(AMP)');xlabel('Time(MIN)');
hold on; plot(x_est4(:,1:DAY),'r','linewidth',2); legend('True','Estimate');

subplot(5,2,5), plot(x5(:,st:en),'linewidth',2),title(' Cloths Washer'), ylabel('Current(AMP)');xlabel('Time(MIN)');
hold on; plot(x_est5(:,1:DAY),'r','linewidth',2); legend('True','Estimate');

subplot(5,2,6), plot(x6(:,st:en),'linewidth',2),title('Dining Room plugs'),ylabel('Current(AMP)');xlabel('Time(MIN)');
hold on; plot(x_est6(:,1:DAY),'r','linewidth',2); legend('True','Estimate');

subplot(5,2,7), plot(x7(:,st:en),'linewidth',2),title('Dishwasher'),ylabel('Current(AMP)');xlabel('Time(MIN)');
hold on; plot(x_est7(:,1:DAY),'r','linewidth',2); legend('True','Estimate');

subplot(5,2,8), plot(x8(:,st:en),'linewidth',2),title('Electronics Workbench'),ylabel('Current(AMP)');xlabel('Time(MIN)');
hold on; plot(x_est8(:,1:DAY),'r','linewidth',2); legend('True','Estimate');

subplot(5,2,9), plot(x17(:,st:en),'linewidth',2),title('TV and Entertainment'),ylabel('Current(AMP)');xlabel('Time(MIN)');
hold on; plot(x_est17(:,1:DAY),'r','linewidth',2); legend('True','Estimate');

subplot(5,2,10), plot(x18(:,st:en),'linewidth',2),title('Utility Room Plug'),ylabel('Current(AMP)');xlabel('Time(MIN)');
